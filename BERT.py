...
def get_texts_labels(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            record = json.loads(
                line.strip())  # parse each line from a JSON-formatted string into a dictionary after removing leading and trailing whitespace or newline characters
...
class BERT_Text_Classifier(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.bert = BertModel.from_pretrained(
            'bert-base-uncased')  # load pre-trained BERT model # NOTE GETTING A WARNING ABOUT NEEDING RENAME WEIGHTS AND BIAS!!
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask,
                            token_type_ids=token_type_ids)  # pass input through BERT (feedforward) DOCUMENTATION
        cls_output = outputs[
            1]  # get [CLS] token output for classification, it is a pooled output (summary of the input sequence) for classification  DOCUMENTATION
...
train_data = SemEval_train_data  # chosen train data (SemEval or GenAI)
train_label_data = SemEval_train_label_data  # chosen (labels of) train data (SemEval or GenAI)
val_data = SemEval_val_data  # chosen val data (SemEval or GenAI)
val_label_data = SemEval_val_label_data  # chosen (labels of) val data (SemEval or GenAI)
batch_size = 64
train_dataloader = DataLoader(
    list(zip(train_data['input_ids'], train_data['token_type_ids'], train_data['attention_mask'], train_label_data)),
    batch_size=batch_size,
    shuffle=True)  # Here please change batch size depending of your GPU capacities (if GPU runs out of memory lower batch_size)
...
val_dataloader = DataLoader(
    list(zip(val_data['input_ids'], val_data['token_type_ids'], val_data['attention_mask'], val_label_data)),
    batch_size=batch_size,
    shuffle=True)  # Here please change batch size depending of your GPU capacities (if GPU runs out of memory lower batch_size)
    ...
    with torch.no_grad():  # no update to gradient (freezes weights), because we don't train model on val data (only want to extract predictions for testing)
        for input_ids, token_type_ids, attention_mask, label in tqdm(val_dataloader):
            input_ids, token_type_ids, attention_mask, label = input_ids.to(device), token_type_ids.to(
                device), attention_mask.to(device), label.to(device)  # moves data to device

            log_probs = model(input_ids, attention_mask,
                              token_type_ids)  # feedforward: run forward function of model on input texts and outputs 2 logits (for each class) per text/sample in batch
            _, predictions = torch.max(log_probs,
                                       dim=1)  # get predictions - finds the index of the maximum logit value which is the index of our predicted class (0 or 1)
            correct += (
                        predictions == label).sum().item()  # count how many correct predictions have been made for current batch
            total += len(
                label)  # count how many total predictions have been made, which is basically the length of label tensor across batches
...
model = BERT_Text_Classifier(2).to(
    device)  # moves initialized model (binary classifier where Machine text = label 0 and Human text = label 1) to device
...
for epoch in range(epochs):  # for each epoch
    ...
    for input_ids, token_type_ids, attention_mask, label in tqdm(
            train_dataloader):  # for each batch of (surrounding, target) pairs
        ...
        log_probs = model(input_ids, attention_mask,
                          token_type_ids)  # feedforward: run forward function of model on input texts and outputs and outputs 2 logits (for each class) per text/sample in batch

        loss = loss_function(log_probs,
                             label)  # criterion - calculate loss between prediction and target, note CrossEntropyLoss function expects target values to be indices and first converts logits of outputs to probs using softmax to calculate: -Σylog(p)
    ...
    accuracy = evaluate(model, val_dataloader,
                        device)  # evaluate (via accuracy) after each epoch using SemEval validation set
